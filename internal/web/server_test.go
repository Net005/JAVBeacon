...
func TestEmbeddedFrontendIncludesGlobalZoomAndLocalScreenshotUI(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`detailFilterList([x.director],'Director')`,
		`<dt>Director</dt><dd>${filterButton(x.director,'Director')||'—'}</dd>`,
		`['director','Director']`,
		`director:'conditionDirectorOptions'`,
		`id="discoveryStatusFold"`,
		`javbeacon.discoveryStatusOpen`,
		`renderDiscoveryJobWithoutAutoFold`,
