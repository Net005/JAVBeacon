...
func TestReleaseLabelAndDownloadStatus(t *testing.T) {
	...
	dl, err := s.SaveDownload(ctx, domain.Download{ReleaseID: got.ID, Provider: "JavDB / Keepshare", SourceReference: "https://keepshare.org/example", TransferReference: "https://download.invalid/signed", ProviderFileID: "pikpak-file-3400", RestoredFileID: "restored-pikpak-file-3400", RestoredParentID: "pack-from-shared-3400", RestoredFileOwned: true, Query: "LBL-1", Transport: "http", Status: "downloading"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || len(storedDownloads) != 1 || storedDownloads[0].TransferReference != "https://download.invalid/signed" || storedDownloads[0].ProviderFileID != "pikpak-file-3400" || storedDownloads[0].RestoredFileID != "restored-pikpak-file-3400" || storedDownloads[0].RestoredParentID != "pack-from-shared-3400" || !storedDownloads[0].RestoredFileOwned {
		t.Fatalf("provider/restored file identity was not persisted: downloads=%+v err=%v", storedDownloads, err)
	}
	dl.Status = "completed"
	if _, err := s.SaveDownload(ctx, dl); err != nil {
		t.Fatal(err)
	}
	if got := fetch(); got.DownloadStatus != "completed" || got.DownloadTransport != "http" || got.DownloadSourceReference != "https://keepshare.org/example" {
		t.Fatalf("expected linked HTTP completed status: %+v", got)
	}
...
func TestReleaseDownloadedAtAndStashAddedAt(t *testing.T) {
	...
	dl, err := s.SaveDownload(ctx, domain.Download{ReleaseID: got.ID, Provider: "Sukebei", Query: "DATE-1", Status: "downloading", SourceReference: "https://sukebei.nyaa.si/view/123", BytesPerSecond: 1048576, Seeds: 23, Peers: 4, ETASeconds: 321, SeenComplete: 1700000000})
	if err != nil {
		t.Fatal(err)
	}
	latest, err := s.LatestReleaseDownload(ctx, got.ID)
	if err != nil || latest.ID != dl.ID || latest.BytesPerSecond != 1048576 || latest.Seeds != 23 || latest.Peers != 4 || latest.ETASeconds != 321 || latest.SeenComplete != 1700000000 || latest.AddedAt.IsZero() {
