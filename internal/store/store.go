...
type Store interface {
	...
	BulkResetIgnoreLocalForceDownload(context.Context, []int64) (int64, error)
	BulkSetReleaseDownloadOverrides(context.Context, []int64, string, bool, bool) (int64, error)
...
CREATE INDEX IF NOT EXISTS idx_release_tags_release_position ON release_tags(release_id,position);`)
	...
	if err == nil {
		...
		for _, statement := range []string{
			...
			`ALTER TABLE downloads ADD COLUMN transfer_reference TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE downloads ADD COLUMN source_page_url TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE downloads ADD COLUMN provider_file_id TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE downloads ADD COLUMN restored_file_id TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE downloads ADD COLUMN restored_parent_id TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE downloads ADD COLUMN restored_file_owned INTEGER NOT NULL DEFAULT 0`,
...
func releaseFilterWhere(d Dialect, f domain.ReleaseFilter) (string, []any) {
	...
	if f.StashWatched {
		q += ` AND r.is_local=1 AND r.stash_scene_id<>'' AND r.play_count>0`
	}
...
func (s *SQLite) PatchRelease(ctx context.Context, id int64, released, local, notified, notifyOnRelease, watchlist, monitorDownload *bool, label *string, ignoreLocalForceDownload *bool, httpPrimary ...*bool) error {
	now := time.Now().UTC()
	...
	a := []any{now}
	for k, v := range map[string]*bool{"released": released, "is_local": local, "notified": notified, "notify_on_release": notifyOnRelease, "watchlist": watchlist, "monitor_download": monitorDownload, "ignore_local_force_download": ignoreLocalForceDownload, "http_download_primary": httpDownloadPrimary} {
		if v != nil {
			sets = append(sets, k+"=?")
			a = append(a, *v)
		}
...
// ignore_local_force_download to every release in ids in a single
// statement - the mass-select "stop monitoring" and "ignore StashApp
// Local / force download" bulk actions on the "Releases checked by the
// scheduled job" table. Any pointer may be nil to leave that column
// untouched; all nil (or an empty ids) is a no-op.
func (s *SQLite) BulkSetReleaseFlags(ctx context.Context, ids []int64, monitorDownload, ignoreLocalForceDownload *bool, httpPrimary ...*bool) (int64, error) {
	var httpDownloadPrimary *bool
	if len(httpPrimary) > 0 {
		httpDownloadPrimary = httpPrimary[0]
	}
	if len(ids) == 0 || (monitorDownload == nil && ignoreLocalForceDownload == nil && httpDownloadPrimary == nil) {
		return 0, nil
	}
	sets := []string{"updated_at=?"}
	a := []any{time.Now().UTC()}
...
// BulkResetIgnoreLocalForceDownload clears IgnoreLocalForceDownload for
// every given release id, and - for any release that is already matched in
// StashApp (is_local=1) - also clears MonitorDownload/MonitorReason/
// MonitorSiteID in the same statement. This mirrors SetStashState's own
// not-local-to-local auto-unmonitor transition: a release whose "keep
// searching despite the StashApp match" override is being turned off, and
// that already has a local match, has no remaining reason to stay
// monitored. A release that is not local yet keeps whatever monitoring
// state it already had - clearing the override alone does not touch it.
func (s *SQLite) BulkResetIgnoreLocalForceDownload(ctx context.Context, ids []int64) (int64, error) {
	if len(ids) == 0 {
		return 0, nil
	}
	a := []any{time.Now().UTC()}
	...
	r, e := s.db.ExecContext(ctx, `UPDATE releases SET ignore_local_force_download=0,updated_at=?,monitor_download=CASE WHEN is_local=1 THEN 0 ELSE monitor_download END,monitor_reason=CASE WHEN is_local=1 THEN '' ELSE monitor_reason END,monitor_site_id=CASE WHEN is_local=1 THEN 0 ELSE monitor_site_id END WHERE id IN (`+strings.Join(placeholders, ",")+`)`, a...)
	if e != nil {
		return 0, e
	}
	return r.RowsAffected()
...
func (s *SQLite) BulkSetReleaseDownloadOverrides(ctx context.Context, ids []int64, method string, ignoreLocal, ignoreHistory bool) (int64, error) {
	...
	args := []any{method, ignoreLocal, ignoreHistory, time.Now().UTC()}
	placeholders := make([]string, len(ids))
	for i, releaseID := range ids {
		placeholders[i] = "?"
		args = append(args, releaseID)
	}
	result, err := s.db.ExecContext(ctx, `UPDATE releases SET download_method_override=?,ignore_local_force_download=?,ignore_download_history=?,updated_at=? WHERE id IN (`+strings.Join(placeholders, ",")+`)`, args...)
	if err != nil {
		return 0, err
	}
...
func (s *SQLite) SaveDownload(ctx context.Context, x domain.Download) (domain.Download, error) {
	...
	if x.ID == 0 {
		var err error
		x.ID, err = s.dialect.InsertReturningID(ctx, s.db, `INSERT INTO downloads(release_id,provider,source_type,source_reference,transfer_reference,source_page_url,provider_file_id,restored_file_id,restored_parent_id,restored_file_owned,query,torrent_hash,transport,destination_path,bytes_total,bytes_downloaded,bytes_per_second,name,files,status,match_reason,qb_response,post_status,error,seed_ratio,progress,seeds,peers,eta_seconds,seen_complete,filename_pattern_excluded,priority,added_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, x.ReleaseID, x.Provider, x.SourceType, x.SourceReference, x.TransferReference, x.SourcePageURL, x.ProviderFileID, x.RestoredFileID, x.RestoredParentID, x.RestoredFileOwned, x.Query, x.TorrentHash, x.Transport, x.DestinationPath, x.BytesTotal, x.BytesDownloaded, x.BytesPerSecond, x.Name, string(x.Files), x.Status, x.MatchReason, x.QBResponse, x.PostStatus, x.Error, x.SeedRatio, x.Progress, x.Seeds, x.Peers, x.ETASeconds, x.SeenComplete, x.FilenamePatternExcluded, x.Priority, now, now)
		if err != nil {
			return x, err
		}
		x.AddedAt = now
	} else {
		_, err := s.db.ExecContext(ctx, `UPDATE downloads SET transfer_reference=?,source_page_url=?,provider_file_id=?,restored_file_id=?,restored_parent_id=?,restored_file_owned=?,torrent_hash=?,transport=?,destination_path=?,bytes_total=?,bytes_downloaded=?,bytes_per_second=?,name=?,files=?,status=?,match_reason=?,qb_response=?,post_status=?,error=?,seed_ratio=?,progress=?,seeds=?,peers=?,eta_seconds=?,seen_complete=?,filename_pattern_excluded=?,priority=?,updated_at=? WHERE id=?`, x.TransferReference, x.SourcePageURL, x.ProviderFileID, x.RestoredFileID, x.RestoredParentID, x.RestoredFileOwned, x.TorrentHash, x.Transport, x.DestinationPath, x.BytesTotal, x.BytesDownloaded, x.BytesPerSecond, x.Name, string(x.Files), x.Status, x.MatchReason, x.QBResponse, x.PostStatus, x.Error, x.SeedRatio, x.Progress, x.Seeds, x.Peers, x.ETASeconds, x.SeenComplete, x.FilenamePatternExcluded, x.Priority, now, x.ID)
		if err != nil {
			return x, err
		}
	}
...
func scanDownloads(rows *sql.Rows) ([]domain.Download, error) {
	...
	for rows.Next() {
		...
		var files string
		if err := rows.Scan(&x.ID, &x.ReleaseID, &x.VideoID, &x.ImageURL, &x.Provider, &x.SourceType, &x.SourceReference, &x.TransferReference, &x.SourcePageURL, &x.ProviderFileID, &x.RestoredFileID, &x.RestoredParentID, &x.RestoredFileOwned, &x.Query, &x.TorrentHash, &x.Transport, &x.DestinationPath, &x.BytesTotal, &x.BytesDownloaded, &x.BytesPerSecond, &x.Name, &files, &x.Status, &x.MatchReason, &x.QBResponse, &x.PostStatus, &x.Error, &x.SeedRatio, &x.Progress, &x.Seeds, &x.Peers, &x.ETASeconds, &x.SeenComplete, &x.FilenamePatternExcluded, &x.Priority, &x.AddedAt, &x.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *SQLite) DownloadActivity(ctx context.Context, f domain.DownloadFilter) ([]domain.Download, int, error) {
	...
	sortColumn := map[string]string{"added": "d.added_at", "status": "d.status", "seeds": "d.seeds", "peers": "d.peers", "seen_complete": "d.seen_complete", "eta": "d.eta_seconds", "progress": "d.progress", "priority": "d.priority"}[f.Sort]
	if sortColumn == "" {
