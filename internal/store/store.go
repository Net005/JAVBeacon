...
func (s *SQLite) SaveDownload(ctx context.Context, x domain.Download) (domain.Download, error) {
	...
	if x.ID == 0 {
		var err error
		x.ID, err = s.dialect.InsertReturningID(ctx, s.db, `INSERT INTO downloads(release_id,provider,source_type,source_reference,source_page_url,provider_file_id,restored_file_id,restored_parent_id,restored_file_owned,query,torrent_hash,transport,destination_path,bytes_total,bytes_downloaded,bytes_per_second,name,files,status,match_reason,qb_response,post_status,error,seed_ratio,progress,seeds,peers,eta_seconds,seen_complete,filename_pattern_excluded,added_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, x.ReleaseID, x.Provider, x.SourceType, x.SourceReference, x.SourcePageURL, x.ProviderFileID, x.RestoredFileID, x.RestoredParentID, x.RestoredFileOwned, x.Query, x.TorrentHash, x.Transport, x.DestinationPath, x.BytesTotal, x.BytesDownloaded, x.BytesPerSecond, x.Name, string(x.Files), x.Status, x.MatchReason, x.QBResponse, x.PostStatus, x.Error, x.SeedRatio, x.Progress, x.Seeds, x.Peers, x.ETASeconds, x.SeenComplete, x.FilenamePatternExcluded, now, now)
		if err != nil {
			return x, err
		}
		x.AddedAt = now
	} else {
		_, err := s.db.ExecContext(ctx, `UPDATE downloads SET source_page_url=?,provider_file_id=?,restored_file_id=?,restored_parent_id=?,restored_file_owned=?,torrent_hash=?,transport=?,destination_path=?,bytes_total=?,bytes_downloaded=?,bytes_per_second=?,name=?,files=?,status=?,match_reason=?,qb_response=?,post_status=?,error=?,seed_ratio=?,progress=?,seeds=?,peers=?,eta_seconds=?,seen_complete=?,filename_pattern_excluded=?,updated_at=? WHERE id=?`, x.SourcePageURL, x.ProviderFileID, x.RestoredFileID, x.RestoredParentID, x.RestoredFileOwned, x.TorrentHash, x.Transport, x.DestinationPath, x.BytesTotal, x.BytesDownloaded, x.BytesPerSecond, x.Name, string(x.Files), x.Status, x.MatchReason, x.QBResponse, x.PostStatus, x.Error, x.SeedRatio, x.Progress, x.Seeds, x.Peers, x.ETASeconds, x.SeenComplete, x.FilenamePatternExcluded, now, x.ID)
		if err != nil {
			return x, err
		}
	}
...
func scanDownloads(rows *sql.Rows) ([]domain.Download, error) {
	...
	for rows.Next() {
		...
		var files string
		if err := rows.Scan(&x.ID, &x.ReleaseID, &x.VideoID, &x.ImageURL, &x.Provider, &x.SourceType, &x.SourceReference, &x.SourcePageURL, &x.ProviderFileID, &x.RestoredFileID, &x.RestoredParentID, &x.RestoredFileOwned, &x.Query, &x.TorrentHash, &x.Transport, &x.DestinationPath, &x.BytesTotal, &x.BytesDownloaded, &x.BytesPerSecond, &x.Name, &files, &x.Status, &x.MatchReason, &x.QBResponse, &x.PostStatus, &x.Error, &x.SeedRatio, &x.Progress, &x.Seeds, &x.Peers, &x.ETASeconds, &x.SeenComplete, &x.FilenamePatternExcluded, &x.AddedAt, &x.UpdatedAt); err != nil {
			return nil, err
		}
