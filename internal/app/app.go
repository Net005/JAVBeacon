...
func finishStartup(cfg config.Config, log *slog.Logger, logs *logging.RingHandler, st *store.SQLite) (*App, error) {
	...
	if len(settings) == 0 {
		_ = st.SaveSettings(context.Background(), map[string]string{"page_limit": fmt.Sprint(defaultPageLimit), "refresh_interval": cfg.RefreshText, "recent_limit": "200", "hide_local": "false", "sort": "release", "view": "grid", "notification_sort": "added", "flaresolverr_url": cfg.FlareSolverrURL, "flaresolverr_cooldown": fmt.Sprint(cfg.FlareSolverrCooldown), "byparr_request_timeout_seconds": "30", "byparr_solve_timeout_seconds": "75", "cover_directory": cfg.CoverDirectory, "session_lifetime": "720h", "notification_interval": "15m", "rss_interval": "5m", "search_url_template": "https://sukebei.nyaa.si/?page=rss&f=0&c=2_0&q=<release_id>", "accepted_patterns": download.DefaultPreferredFilenamePatterns(), "search_download_background": "false", "minimum_seed_ratio": "1.0", "qb_poll_interval_seconds": "15", "qb_completed_action": "remove_at_ratio", "javdb_url": "https://javdb.com", "javdb_request_timeout_seconds": "90", "pikpak_resolution_timeout_seconds": "600", "http_download_concurrency": "1", "http_download_connections": "4", "http_fallback_delay": "8h", "default_download_method": "torrent_http", "prefer_http_equivalent": "true", "pikpak_release_id_folder_fallback": "false", "javdb_gluetun_rotation_enabled": "false", "gluetun_control_url": "http://127.0.0.1:8000", "gluetun_rotation_attempts": "3", "gluetun_rotation_wait_seconds": "45", "gluetun_rotation_poll_milliseconds": "1000", "gluetun_rotation_settle_seconds": "2", "gluetun_require_ip_change": "true"})
	}
	missing := map[string]string{}
	if raw := settings["accepted_patterns"]; raw != "" {
		if normalized := download.NormalizePreferredFilenamePatterns(raw); normalized != raw {
			missing["accepted_patterns"] = normalized
		}
	}
	if settings["javdb_url"] == "" {
		missing["javdb_url"] = "https://javdb.com"
	}
	for key, value := range map[string]string{"javdb_request_timeout_seconds": "90", "pikpak_resolution_timeout_seconds": "600"} {
		if settings[key] == "" {
			missing[key] = value
		}
	}
	...
	if settings["default_download_method"] == "" {
		missing["default_download_method"] = "torrent_http"
	}
	...
	for k, v := range map[string]string{
		...
		"discoveries_ai_enabled":                     "false",
		"discoveries_ai_primary_provider":            "ollama",
		"discoveries_ollama_url":                     cfg.OllamaURL,
		...
		"discoveries_openai_batch_size":              "5",
		"discoveries_openai_max_input_chars":         "50000",
		"discoveries_openai_monthly_budget":          "5",
		"discoveries_openai_batch":                   "true",
		"discoveries_ai_last_provider":               "",
		"discoveries_ai_last_model":                  "",
		"discoveries_ai_last_input_tokens":           "0",
		"discoveries_ai_last_output_tokens":          "0",
		"discoveries_ai_last_estimated_cost_usd":     "0",
		"discoveries_subtitle_analysis_enabled":      "false",
		"discoveries_subtitle_languages":             "en, ja",
		"discoveries_subtitle_max_chars":             "4000",
